...
export default function PurchaseInvoicesNew() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50 p-6 ${isRTL ? 'rtl' : 'ltr'}`}>
      ...
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-4xl font-bold text-slate-800 mb-2">{getText('title')}</h1>
      </motion.div>
      ...
      >
        <Button
          ...
          className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2"
        >
      ...
      <motion.div
        ...
        className="mt-8"
      >
        <h2 className="text-2xl font-bold mb-4">{language === 'ar' ? 'الفواتير الأخيرة' : 'Recent Invoices'}</h2>
        <div className="grid gap-4">
          {invoices.slice(0, 5).map((invoice) => (
            <Card key={invoice.id} className="border-l-4 border-l-blue-500 hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-bold text-blue-600">💵 {invoice.total_amount}</p>
