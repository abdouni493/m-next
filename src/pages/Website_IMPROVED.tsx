...
export default function Website() {
  ...
  const handleCreateOffer = async () => {
    if (!selectedProductOffer) return;
    ...
    try {
      ...
      toast({
        title: language === 'ar' ? 'خطأ' : 'Erreur',
        description: language === 'ar' ? 'فشل في إنشاء العرض' : 'Impossible de créer l\'offre',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleCreateSpecialOffer = async () => {
    if (!selectedProductSpecial) return;
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="text-5xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
                {language === 'ar' ? '🌐 إدارة الموقع' : '🌐 Gestion Web'}
              </h1>
              <p className="text-slate-600 dark:text-slate-400 mt-1">
                {language === 'ar' ? 'تحكم كامل في عروضك والتواصل' : 'Contrôle complet de vos offres'}
              </p>
            </div>
          </div>
        </motion.div>
